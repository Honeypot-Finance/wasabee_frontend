...
console.log('REDIS_CONNECTION_STRING', REDIS_CONNECTION_STRING)
...
// })
const cacheStore = bentostore().useL1Layer(
  memoryDriver({ maxItems: 3000, maxSize: 10_000_000 })
)
if (REDIS_CONNECTION_STRING) {
  cacheStore.useL2Layer(
    redisDriver({
      connection: new Redis(REDIS_CONNECTION_STRING!),
    }),
  )
}
export const bento =
