...
if (!process.env.BOOGAOOGA_API_KEY) throw new Error("API_KEY is required");

const PRIVATE_KEY = process.env.PRIVATE_KEY as Address; // Private key of the account to make the trade
const BOOGAOOGA_PUBLIC_API_URL = process.env.BOOGAOOGA_PUBLIC_API_URL;
const API_KEY = process.env.BOOGAOOGA_API_KEY;
...
const HONEY: Address = "0x7EeCA4205fF31f947EdBd49195a7A88E6A91161B"; // Berachain's native stablecoin
...
const getAllowance = async (token: Address, from: Address) => {
  ...
  const publicApiUrl = new URL(
    `${BOOGAOOGA_PUBLIC_API_URL}/v1/approve/allowance`
  );
...
const swap = async (swapParams: SwapParams) => {
  const publicApiUrl = new URL(`${BOOGAOOGA_PUBLIC_API_URL}/v1/swap`);
  ...
  const hash = await client.sendTransaction({
    ...
    value: tx.value ? BigInt(tx.value) : BigInt("0n"),
  });
...
const approveAllowance = async (
  ...
  const publicApiUrl = new URL(`${BOOGAOOGA_PUBLIC_API_URL}/v1/approve`);
  publicApiUrl.searchParams.set("token", token);
  publicApiUrl.searchParams.set("from", from);
