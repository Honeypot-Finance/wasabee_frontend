...
const ApprovalsCard = ({title, buttonTitle}: {title: string,buttonTitle: string}) => {
    return  (
        <div className='py-5 px-7 flex flex-col gap-4 items-center bg-[#211708] border border-[#F7931A1A] rounded-[20px]'>
            <div className='text-[12px] leading-4'>Step 1</div>
            <div>{title}</div>
            <div className='w-36 h-[2px] bg-[#37240A]'/>
            <Button styleMode='plain' className='rounded-full outline-0 border-0' disabled={true}>
                {buttonTitle}
            </Button>
        </div>
    )
    ...
    <div>
        <div className='text-xl font-medium'>Quick Summary</div>
            <div className='flex flex-col gap-9'>
                ...
                <div>
                    <div className='text-base leading-5 font-medium pl-[10px]'>Final Approvals</div>
                    <div className='flex gap-3.5 mt-3.5'>
                        <ApprovalsCard title={'Approve USDC'} buttonTitle="Approve"/>
                        <ApprovalsCard title={'Approve USDC'} buttonTitle="Approve"/>
                    </div>
                </div>
        </div>
