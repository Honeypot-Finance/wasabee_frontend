...
export const Button = ({children, className,isLoading, ...props}: {
    ...
    return isConnected ? <NextButton isLoading={isLoading} className={clsx(baseClassNames)}  {...props}>{children}</NextButton> : <NextButton className={clsx(baseClassNames)} isLoading={!!connectModalOpen} {...props} onClick={() => {
        openConnectModal?.()
