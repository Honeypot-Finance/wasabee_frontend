...
export const SimplePriceFeedGraph = observer((props: Props) => {
  ...
  useEffect(() => {
    if (props.priceFeedTarget) {
      const tokenNum =
        props.priceFeedTarget instanceof PairContract
          ? swap.fromToken?.address == props.priceFeedTarget.token0.address
            ? 0
            : 1
          : 0;
      const currencyCode: "TOKEN" | "USD" =
        props.priceFeedTarget instanceof PairContract ? "TOKEN" : "USD";
