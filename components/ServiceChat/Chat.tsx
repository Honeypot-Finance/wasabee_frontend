  ...
  return (
    <div className="flex flex-col h-[90vh] sm:h-[600px] w-full sm:w-[450px] md:w-[550px] border rounded-lg shadow-lg bg-[#3E2A0F] text-[#F5E6D3]">
      ...
      <ScrollArea className="flex-grow p-4" ref={scrollAreaRef}>
        <Card className="mb-4 bg-[#5D4B2C] border-[#FFCD4D] border absolute left-4 right-4 z-10">
          <CardHeader className="p-2 flex flex-row items-center justify-between space-y-0 pb-2 border-b border-[#FFCD4D]">
            <CardTitle className="text-sm font-medium text-[#FFCD4D]">Common Questions</CardTitle>
            <Button
              variant="ghost"
              size="sm"
              className="w-9 p-0 hover:bg-[#7A6236] text-[#FFCD4D]"
              ...
              <div className="flex flex-col space-y-2">
                {presetQuestions.map((question, index) => (
                  <Button
                    ...
                    className="justify-start bg-[#7A6236] text-[#F5E6D3] hover:bg-[#8F7A4F] border-[#FFCD4D]"
                  >
                    {question}
                  </Button>
                ))}
